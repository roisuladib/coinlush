  ...
  const renderCell = useCallback((market: Market['data']['coins'][number], key: React.Key) => {
    ...
    switch (key) {
      case 'name':
        return (
          <div className="flex items-center gap-2">
            ...
            <span>
              {cellValue} /{' '}
              <span className="font-medium text-foreground-500">{market.symbol.toUpperCase()}</span>
            </span>
          </div>
        );
      ...
      case 'change': {
        ...
        return (
          <Chip color={isUp ? 'success' : 'danger'} size="sm" variant="flat">
      ...
      case 'detail':
        return (
          <Tooltip content="Details" showArrow color="foreground">
            <Button
              color="primary"
              variant="flat"
              isIconOnly
        ...
        <div className="flex items-center justify-between">
          <div className="inline-flex w-full justify-end gap-3">
            <Select
              ...
              {e => (
                <SelectItem className="capitalize" key={e.key}>
                  {e.label}
                </SelectItem>
              )}
            </Select>
